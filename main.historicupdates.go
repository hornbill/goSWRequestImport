...
func processHistoricUpdates() {
	logger(1, "Processing Call Diary Entries, please wait...", true)

	var wg sync.WaitGroup

	jobs := make(chan RequestReferences, maxGoroutines)

	for w := 1; w <= maxGoroutines; w++ {
		go applyHistoricalUpdates(jobs, &wg)
	}

	bar := pb.StartNew(len(arrCallsLogged))
	for swCallID, smCallID := range arrCallsLogged {
		wg.Add(1)
		jobs <- RequestReferences{SwCallID: swCallID, SmCallID: smCallID}
		mutexBar.Lock()
		bar.Increment()
		mutexBar.Unlock()
	}
